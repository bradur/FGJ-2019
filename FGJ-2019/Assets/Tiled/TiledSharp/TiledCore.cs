    ...
    public class TmxDocument
    {
        ...
        protected XDocument ReadXml(string filepath)
        {
            ...
            else
            {
                ...
                // hack path
                //filepath = string.Format("{0}{1}{2}", "Resources", System.IO.Path.PathSeparator, filepath);
                //filepath = string.Format("[{0}] [{1}] [{2}]", System.IO.Path.GetPathRoot(filepath), System.IO.Path.GetFullPath(filepath), filepath);
                //filepath = System.IO.Path.GetFullPath(filepath);
                filepath = Tools.ReplaceString(
                    filepath,
                    "../",
                    string.Format("{0}Assets{1}Resources{1}", "../", "/")
                );
